...
<!-- Login form -->
    <form class="login-form ms-auto bg-secondary-subtle mt-4 mb-3 me-auto justify-content-center p-3 border border-3 rounded-3" action="{{route('login_post')}}" method="POST" style="width:400px">
        @csrf
        <!-- Heading -->
        <div class="text-center fst-italic fs-1 justify-content-center text-white"  >Welcome to FoodFusion</div>
        <!-- Email Input -->
        <div class="mb-3 mt-3">
            <label for="UserEmail" class="form-label text-white fw-bolder fs-5">Email</label>
            <input type="email" class="form-control" name="email" id="UserEmail" aria-describedby="emailHelp">  
            <p class="text-danger">{{$errors->first('email')}}</p>
        </div>
        <!-- Password Input -->
        <div class="mb-3">
            <label for="UserPassword" class="form-label text-white fw-bolder fs-5">Password</label>
        ...
        <div class="mb-3 form-check">
            ...
            <label class="form-check-label text-white fw-bolder fs-5" id="l_showpw" for="ShowPw">Show Password</label>
        </div>
        <!-- Submit Button -->
        <button type="submit" id="loginbtn" class="btn btn-primary bg-white text-dark border-dark m-3">LOGIN</button>
    ...
    <script>
        ...
        let l_pw =document.getElementById('UserPassword');
        let l_checkbox= document.getElementById('ShowPw');
        l_checkbox.addEventListener('change',()=>{
